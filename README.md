...
- **Analyzer**: `python -m collector.analyze --input <logs> --out <dir>` (Rate/AoI/MAE 집계 → CSV/MD 리포트 + `figs/` → (옵션) `--audit` QC 리포트)
...
- **Analysis outputs**
  ...
  - `results/run1/report.md` (표 + baseline 비교 + (옵션) `figs/` 임베드)
  - (plots, 기본 ON) `results/run1/figs/` (보고서/논문 삽입용 그림; 기본 `png,pdf`, DPI=300)
    - 네이밍 규칙: `docs/metrics/FIGURE_NAMING.md`
    - 라벨 규칙: `docs/metrics/LABEL_STYLE.md`
  - (paper-plots, 기본 ON) 추가 패널(예): `results/run1/figs/*_env_metrics_panel.*`, `*_action_heatmap.*`, `*_feature_weights__*.*`
    - 비활성화: `python -m collector.analyze ... --no-paper-plots`
  - (plots 생성 시) `results/run1/plot_manifest.json` (라벨 audit용)
  - (audit, 기본 OFF) `--audit` → `results/run1/quality_audit.json`, `results/run1/quality_audit.md`
...
- Rate–AoI **Pareto / trade-off** 시각화(`figs/*_pareto_rate_vs_aoi_mean.*`) + 해석
...
### 1) 1차(메인) 지표 — 표/그래프 필수
- **Rate [B/s]** (`rate_Bps`): 브로커가 수신한 전송량(낮을수록 좋음)
- **AoI mean / p95 [ms]** (`aoi_mean_ms`, `aoi_p95_ms`): 수신자가 보유한 최신 정보의 나이(낮을수록 좋음)
- **MAE_event mean / p95** (`mae_event_mean`, `mae_event_p95`): 이벤트 로그의 `|res|` 기반 오차(낮을수록 좋음)

### 2) 2차(보조) 지표 — 원인 분석/품질 확인용
- **kbits_mean**: 평균 양자화 비트 수(낮을수록 전송량↓, 단 오차↑ 가능)
- **event_rate_hz**: 이벤트 전송 빈도(Hz) (낮을수록 전송량↓)
- **send_ratio**: (추정) 샘플 대비 전송 비율(SEQ gap 기반; 1.0이면 periodic 수준)
- **rx_delay_mean_ms / rx_delay_p95_ms**: 수신 지연(수집기 수신시각 − 센서 타임스탬프; 시간 동기 필요)

### 3) 학습(Adaptive LinUCB) 지표 — 논문용 추가 플롯(`--paper-plots`, `figs/` 하위)
- **reward over time**: sliding window 평균 reward(수렴 시 안정화)
- **cumulative regret (proxy)**: θᵀx 예측 기반의 proxy regret 누적(평탄화 = 학습)
- **feature weights θ(t)**: AoI/res/res_var/loss/q_len 가중치 수렴
- **action vs residual/value**: residual(또는 raw value) 구간별 (τ,kbits) 선택 분포

> 참고: 현재 MAE는 연속 신호 전체 복원이 아니라 **이벤트 로그 기반(event-based)** 지표입니다. 논문/보고서에서는 `MAE_event`로 표기하거나,
> “전체 신호 복원 MAE”가 필요하면 raw-stream 로깅(또는 별도 리컨스트럭션 파이프라인)으로 확장하세요.
...
- [ ] **시각화**: `figs/`의 bar + pareto 이미지(보고서/논문에 바로 삽입 가능) + `quality_audit.md` PASS  
...
## Key Metrics
Main paper metrics (generated by `python -m collector.analyze`):
- **Rate [B/s]** (`rate_Bps`): broker-side bytes/sec (lower is better)
- **AoI mean / p95 [ms]** (`aoi_mean_ms`, `aoi_p95_ms`): freshness at receiver (lower is better)
- **MAE_event mean / p95** (`mae_event_mean`, `mae_event_p95`): event-log `|res|` (lower is better)

Secondary diagnostics (optional but recommended for analysis):
- `kbits_mean`, `event_rate_hz`, `send_ratio` (SEQ-gap estimate), `rx_delay_*` (requires time sync)

Learning diagnostics (adaptive only; `--paper-plots` under `figs/`):
- reward moving average, predicted proxy cumulative regret, feature-weight trace θ(t), action distribution heatmaps
...
- `metrics_summary.csv` / `metrics_by_run.csv` / `metrics_vs_periodic.csv` / `figs/` for paper‑ready tables & plots (prove with `quality_audit.md`)  

...
- **Analysis outputs**
  ...
  - (옵션) `results/run1/figures/*.png` (막대/파레토 플롯; `matplotlib` 설치 시 생성)
  - (옵션, 기본 ON) `results/run1/figures/paper_*.png` (논문용 추가 플롯; `--no-paper-plots`로 비활성화)
...
### 3) 학습(Adaptive LinUCB) 지표 — 논문용 추가 플롯(`paper_*.png`)
...
Learning diagnostics (adaptive only; `paper_*.png`):
